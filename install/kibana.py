...
def install(chosenInterfaceIP):
	...
	if kibanaInstalled == False:
		...
		if cpuArch == 'x86_64':
			os.popen('sudo wget https://artifacts.elastic.co/downloads/kibana/kibana-5.5.0-linux-x86_64.tar.gz 2>&1').read()
			...
			os.popen('sudo wget https://artifacts.elastic.co/downloads/kibana/kibana-5.5.0-linux-x86.tar.gz 2>&1').read()
